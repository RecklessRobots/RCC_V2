...
import BallOfPower from './Ball_of_Power.png';
...
{ 
  ...
  const teamLegends="https://recklessrobots.mypinata.cloud/ipfs/Qmbf4ETyj5aH7ZDrQJde8ZmRZGxsepTufRM7DT9XqKaSRC/";
const teamChibis="https://recklessrobots.mypinata.cloud/ipfs/QmTKu16wVpRgNifCFRhtfLf73KxoaJHE1j15RvurfgTe4c/";
...
const [exp1, setExp1] = useState(0);
  ...
  const [ChibiFromStorage, setChibiFromStorage] = useState('');
  const paddedChibiFromStorage = ChibiFromStorage?.toString().padStart(4, '0');
  ...
  if (ChibiFromStorage !== '') {
  fetch('https://recklessrobots.mypinata.cloud/ipfs/QmaankFtqYMWMkCqPYnp8dcvN7aDzhzwhNSweugeYgi9iz/'+ChibiFromStorage+'.json')
 ...
 useEffect(() => {
  ...
  const GettingData = async () => {
    ...
    const query = new Moralis.Query(Game3);
    ...
    try {
      ...
      if (previousScoreObject) {
        ...
        const newScoreObject = new Game3();
...
interface Game3 extends Moralis.Object<GamesAttributes> {
 
}

const Game3 = Moralis.Object.extend({
  className: 'Games'
...
useEffect(() => {
  const SettingExp = async () => {
    // Query the previous score of the user
    const query = await new Moralis.Query(Game3);
    await query.equalTo('player', address);
...
*/
  return (
    ...
    const NftImage = ({ address, id }: { address: string; id: string }) => {
      const { data, error, isLoading } = useNFT(address, id);
      ...
      return (
        <img
          src={data?.metadata?.image?.replace("ipfs://", "https://recklessrobots.mypinata.cloud/ipfs/")}
...
const Character = (props: { name: string; image: string; attack: AttackType; isAttacking: boolean; healthLost: number }) => {
  const [FirstAidKits, setFirstAidKits]=useState(2);
  const [Skill, setSkill]=useState(0.5);
...
const [Lasers, setLasers]=useState(0);
  ...
  const ChibiFromStorage1 = localStorage.getItem('StoredLegend') || '';
  setChibiFromStorage(ChibiFromStorage1);
 
  const paddedChibiFromStorage = ChibiFromStorage1?.toString().padStart(4, '0');
...
const enemypic=localStorage.getItem('EnemyPic')?.padStart(4, '0') ? localStorage.getItem('EnemyPic')?.padStart(4, '0'): '0001' ;
  return (
  ...
  <img  className={props.healthLost > 0 ? "health-lost" : "somename"}
    width='100%'
    height='auto'
    src={teamLegends+enemypic+".png".replace('ipfs://', 'https://recklessrobots.mypinata.cloud/ipfs/')}
  />
...
const [computerHealth, setComputerHealth] = useState(100 + ((StatsScore || 1) * (0.1 + Math.random() / 4 * 40)));
...
*/
  ...
  useEffect(() => {
    if (paddedChibiFromStorage === "0000") {setPlayerHealth(1)
    }
  }, []);
...
} else if (attackType === AttackType.BallOfLight && Lasers > 0) {
...
if (computerAttack !== AttackType.FirstAidKit && computerAttack !== AttackType.Melee && computerAttack !== playerAttack) {
...
// PLAYER ATTACKS
if (attackType === AttackType.FirstAidKit && HeldModules > 0) {
  FirstAidKits();
  setHeldModules(-1);
  setMessage1("You managed to heal during the combat");
}
...
if (attackType === AttackType.Missiles) {
  if (Math.random() < (Skill + 0.1)) {
    ...
    setMessage1("Devastating hammer attack of " + computerLost.toFixed(0) + " damage!");
  } else {
    setMessage1('The hammer was intercepted by enemy shields');
  }
}
...
if (attackType === AttackType.Miniguns) {
  if (Math.random() < (Skill - 0.1)) {
    ...
    setExp(exp+(computerLost/2));
    setComputerHealth(computerHealth - computerLost);
    setMessage1("The arrows of your heavy crossbow pierce the armor of your enemy, causing " + computerLost.toFixed(0) + " damage!");
  } else {
...
if (attackType === AttackType.BallOfLight && Lasers > 0) {
  computerLost = computerHealth * 0.40;
  setExp(exp+(computerLost/2));
  setComputerHealth(computerHealth - computerLost);
  setMessage1("The Ball of power blasts through the melting armor of the enemy, causing " + computerLost.toFixed(0) + " damage!");
  setLasers(0);
} else {
  setMessage1("");
}
...
if (computerAttack === AttackType.FirstAidKit && EnemyHeldModules > 0) {
...
// OTHER ENEMY ATTACKS
if (computerAttack !== AttackType.FirstAidKit && computerAttack !== AttackType.Melee) {
...
<VStack width={'50%'}>
  ...
  <VStack spacing={2} align="center">
    ...
    <HStack width='100%'>
    <Text width= '50% 'className='PlayerTags'>First aid kits: <br></br>{HeldModules}</Text>  <Text width= '50%' className='PlayerTags'>Lasers: <br></br>{Lasers}</Text>
    </HStack>
      ...
      <Flex width={'full'} fontSize='1vw' justifyContent='space-evenly' >
        <Box  width='fit-content' flexDirection='row' >
          <HStack  justifyContent={'space-evenly'}>
            <button style={{width:'20%', height:'auto'}} className='Fbuttons' disabled={isAttacking || !EnableSkull || Lasers<=0 } onClick={() => attack(AttackType.BallOfLight)}>
              <img src={BallOfPower} alt='BallOfLight' />
            
            </button>
            <button style={{width:'20%', height:'auto'}} className='Fbuttons' disabled={isAttacking} onClick={() => attack(AttackType.Missiles)}>
              <img src={Hammer} alt='ice blast' />
           
            </button>
            <button style={{width:'20%', height:'auto'}} className='Fbuttons' disabled={isAttacking} onClick={() => attack(AttackType.Miniguns)}>
              <img src={Crossbow} alt='Miniguns' />
           
            </button>
            <button style={{width:'20%', height:'auto'}} className='Fbuttons' disabled={isAttacking} onClick={() => attack(AttackType.Melee)}>
              <img src={Katana} alt='Melee' />
            
            </button>
            <button style={{width:'20%', height:'auto'}} className='Fbuttons' disabled={isAttacking} onClick={() => FirstAidKits()}>
