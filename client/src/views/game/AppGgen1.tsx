...
const [exp1, setExp1] = useState(0);
  ...
  const [EnemyPic, setEnemyPic] = useState("1");
  ...
  const [gen1FromStorage, setgen1FromStorage] = useState('');
  const paddedgen1FromStorage = gen1FromStorage?.toString();
  ...
  if (gen1FromStorage !== '') {
  fetch('https://recklessrobots.mypinata.cloud/ipfs/QmXULW4NEaHe3SELWLMHGBQy7MwVN14CRMXAtV2ipLmnpw/'+gen1FromStorage+'.json')
 ...
 useEffect(() => {
  ...
  const GettingData = async () => {
    ...
    const query = new Moralis.Query(Game_Gen1);
    ...
    try {
      ...
      if (previousScoreObject) {
        ...
        const newScoreObject = new Game_Gen1();
...
interface Game_Gen1 extends Moralis.Object<GamesAttributes> {
 
}

const Game_Gen1 = Moralis.Object.extend({
  className: 'Games'
...
useEffect(() => {
  const SettingExp = async () => {
    // Query the previous score of the user
    const query = await new Moralis.Query(Game_Gen1);
    await query.equalTo('player', address);
    ...
    try {
      ...
      if (previousExpObject) {
        ...
        const newExpObject = new Game_Gen1();
...
const [Lasers, setLasers]=useState(0);
  ...
  const gen1FromStorage1 = localStorage.getItem('StoredLegend') || '';
  setgen1FromStorage(gen1FromStorage1);
 
  const paddedgen1FromStorage = gen1FromStorage1?.toString().padStart(4, '0');
...
const one=2;
  ...
  height='auto'
  src={
    paddedgen1FromStorage === "0000"
      ? chooseLegend
      : "https://recklessrobots.mypinata.cloud/ipfs/QmZ7L1GAnp5VFrHT4ayvG33x8HS5LHr3p2DxdHy3Qwgs4K/" +
        paddedgen1FromStorage +
...
*/
  ...
  useEffect(() => {
    if (paddedgen1FromStorage === "0000") {setPlayerHealth(1)
    }
  }, []);
